...
def image_search(source_directory, recursive, image_format):
    ...
    if recursive:
        images: list = list(pathlib.Path(source_directory).rglob(image_format, case_sensitive=False))
    else:
        images: list = list(pathlib.Path(source_directory).glob(image_format, case_sensitive=False))
